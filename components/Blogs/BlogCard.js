...
const BlogCard = ({ data }) => {
  return (
    <Link href={`/blogs/${data?.title}`} className="blog-card-container">
      <ImageDisplay
      ...
      <div className="flex align-items-center ">
        {data?.tags?.length &&
          data?.tags.map((data, index) => (
            <div
              key={index}
              className={`inline-flex text-center py-1 px-3 rounded-full transition duration-150 ease-in-out my-2 me-2 ${setTagColor(
                data
              )}`}
            >
